...
// y axis
const STAT_Y_AXIS_SCALE = linearScale_helper(
  0,
  DATA_LENGTH - 1,
  BUFFER,
  SVG_H_R1 - BUFFER
...
// x axis
const STAT_X_AXIS_SCALE = linearScale_helper(
  0,
  d3.max(max_value),
  BUFFER,
  SVG_W_R1 - BUFFER
...
const STAT_X_AXIS_G = group_helper(STAT_SVG, 'statAxis', 0, SVG_H_R1 - BUFFER);
...
SALES_DATA.forEach((obj, index) => {
  d3.select(`#stat${index}`)
    .selectAll('rect')
    .data([
      minOrMax_helper(SALES_DATA[index], 'min', 0),
...
// y axis
const SEAR_Y_AXIS_SCALE = linearScale_helper(
  d3.max(max_value),
  0,
  BUFFER,
  SVG_H_R2 - BUFFER
...
// x axis
const SEAR_X_AXIS_SCALE = linearScale_helper(
  0,
  entries.length - 1,
  BUFFER,
  SVG_W_R2 - BUFFER
);
const SEAR_X_AXIS = axis_helper('bottom', 0, SEAR_Y_AXIS_SCALE);
...
SEAR_SVG.append('g')
  ...
  .attr('x', (d, i) => SEAR_X_AXIS_SCALE(i))
  .attr('y', SVG_H_R2 - 5)
  .style('fill', 'gray')
...
const TRAN_Y_AXIS_SCALE = linearScale_helper(sum, 0, BUFFER, SVG_H_R2 - BUFFER);
...
const TRAN_X_AXIS_SCALE = linearScale_helper(
  0,
  grouped.length - 1,
  BUFFER,
  SVG_W_R2 - BUFFER
...
TRAN_SVG.selectAll('rect')
  ...
  .attr('y', (d, i) =>
    linearScale_helper(sum, 0, BUFFER, SVG_H_R2 - 2 * BUFFER)(grouped_helper(d))
  )
...
// y axis
const SETS_Y_AXIS_SCALE = linearScale_helper(
...
// x axis
const SETS_X_AXIS_SCALE = linearScale_helper(
  0,
