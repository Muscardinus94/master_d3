...
PAIR_G.each((d, i, n) => {
  d3.select(n[i])
    ...
    .attr('height', () => SVG_HEIGHT - BUFFER - Y_AXIS_SCALE(d))
    .attr(
      'x',
      () =>
...
document.querySelector(
  '#note p'
).innerHTML = `Note: Y-Axis is the count | X-Axis is the week | Average search count for the entire period was: <b>${Math.round(
  d3.mean(D3_JS_DATA)
)}</b>`;
// ADD LEGEND
document.querySelector('#below').style.backgroundColor = COLOR(
  Math.round(d3.mean(D3_JS_DATA)) - 1
);
document.querySelector('#above').style.backgroundColor = COLOR(
  Math.round(d3.mean(D3_JS_DATA))
);
