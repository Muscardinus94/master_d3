...
// .domain
xScale = d3.scaleLinear().domain([50, 100]);
...
xScale = d3.scaleLinear().domain([20, 80]).range([0, svgWidth]);
...
xScale = d3.scaleLinear().domain([20, 80, 140]).range([0, 150, svgWidth]);
...
// .rangeRound()
let x1Scale = d3.scaleLinear().domain([10, 100]).rangeRound([40, 50]);

// .clamp(boolean)
xScale = d3.scaleLinear().domain([20, 30, 60]).range([0, 150, svgWidth]);
xScale.clamp(true);

// .unknown()
xScale.unknown('THINK');
...
// .nice(count(optional))
let x2Scale = d3.scaleLinear().domain([1.256, 7.345]).range([20, 40]);
x2Scale.nice();

console.log(x2Scale.ticks());

// .copy()
let x3Scale = xScale.copy();
