...
// console.log(SVG_WIDTH, SVG_HEIGHT, DATA_LENGTH, densityList, POP_DEN_MAX);
...
document.querySelector('#scalesubmit').addEventListener('click', function (e) {
  const INPUT = document.querySelector('#scaleinput').value;
  if (INPUT >= 0 && INPUT <= 100) {
    document.querySelector('#scaleoutput').innerText = POP_DEN_LIN_SCALE.invert(
      (INPUT / 100) * SVG_WIDTH
    ).toFixed(2);
  }
});
