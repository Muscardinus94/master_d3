...
list.on('click', (event) => {
  indian9IT.forEach((company) => {
    if (event.target.className == company.code) {
      ...
      d3
        .select('#salesgrowth')
        .node().innerHTML = `Sales Growth (3-year): <span>${company.salesGrowth.toLocaleString()}%</span>`;
...
const SVGHEIGHT = d3.selectAll('svg').attr('height');
...
function updateEmployeeChart(companies) {
  if (companies.length == 0) {
    ...
    setTitle('#employeechart', 'Emplyees');
...
function updateGrowthChart(companies) {
  if (companies.length == 0) {
    ...
    setTitle('#growthchart', 'Sales Growth Rate for 3 yesrs(%)');
